...
* `name` - (Required) The name for this HTTP Route Config. Must be between 3 and 63 characters, consisting of lower case letters and digits, starting with a letter. Changing this forces a new resource to be created.
...
* `match` - (Required) A `match` block as defined below.
...
* `prefix_rewrite` - (Optional) The prefix to rewrite the path with.
...
A Container App Environment HTTP Route Config can be imported using the `resource id`, e.g.
